...
public final class SwtResourceUtil {
	public static Font BoldFont = new Font(Display.getCurrent(),
			new FontData[] { new FontData("Arial", 10, 1) });
	private static Set compositeClasses = new HashSet();
	...
	public static KeyStroke getAssitantKey() {
		...
		if (Workbench.getInstance() != null) {
			IBindingService t_BindingService = (IBindingService) PlatformUI
					.getWorkbench().getAdapter(IBindingService.class);
			TriggerSequence t_TriggerSequence = t_BindingService
		...
		try {
			...
			t_KeyStroke = KeyStroke.getInstance(16777235);
		}
	...
	public static boolean equals(KeyStroke r_KeyStroke, KeyEvent r_KeyEvent) {
		if ((r_KeyStroke.getModifierKeys() == 0)
				&& (r_KeyStroke.getNaturalKey() == r_KeyEvent.character)) {
			return true;
		}

		return ((r_KeyStroke.getNaturalKey() == r_KeyEvent.keyCode) && ((r_KeyStroke
				.getModifierKeys() & r_KeyEvent.stateMask) == r_KeyStroke
				.getModifierKeys()));
	}
	...
	public static void setReadonly(Widget r_Control, boolean r_Readonly) {
		...
		if (r_Control instanceof Composite) {
			...
			for (int i = 0; i < t_Controls.length; ++i) {
				...
				setReadonly(t_Control, r_Readonly);
			}
			return;
		}
	}
	...
	public static void setEnable(Control r_Control, boolean r_Enable) {
		...
		} else if (!(r_Control instanceof Label)) {
			r_Control.setEnabled(r_Enable);
		}
	}

	private static void updateColor(Control r_Control, boolean r_Enable) {
	}

	public static ImageDescriptor loadIcon(String r_BundleName,
			String r_ResourceName) {
		ImageRepository t_ImageRepository = ImageRepository
				.getImageRepository(r_BundleName);
		ImageDescriptor t_ImageDescriptor = t_ImageRepository
				.getImageDescriptor(r_ResourceName);

		if (t_ImageDescriptor == null) {
			try {
				URL t_URL = SwtResourceUtil.class.getClassLoader().getResource(
						r_ResourceName);

				if (t_URL == null) {
					t_ImageRepository.setImageDescriptor(r_ResourceName,
							t_ImageDescriptor);
				}
