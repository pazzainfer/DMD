...
public class ExportOracleDllAction extends Action {

	public static final String ID = "com.leven.dmd.pro.nav.action.db.ExportOracleDllAction"; //$NON-NLS-1$

	private static final String[] EXTENSIONS = new String[] { "*.dll", "*.sql", "*.txt"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	
	private Object obj;

	public ExportOracleDllAction(Object obj) {
		this.obj = obj;
		setText(Messages.ExportOracleDllAction_4);
		this.setToolTipText(Messages.ExportOracleDllAction_4);
		setId(ID);
	...
	public void run() {
		...
		try {
			...
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getShell(), getText(), Messages.ExportOracleDllAction_6);
			return;
		}
		...
		if(((INavigatorTreeNode)obj).getNodeType()==INavigatorNodeTypeConstants.TYPE_SCHEMA_FILE){
			...
			fileContent = DbTableUtil.getSchemaDllContent((Schema)root,"oracle"); //$NON-NLS-1$
		} else if(((INavigatorTreeNode)obj).getNodeType()==INavigatorNodeTypeConstants.TYPE_PACKAGE_NODE){
			fileContent = DbTableUtil.getPackageDllContent((TablePackage)((INavigatorTreeNode)obj).getData(),"oracle"); //$NON-NLS-1$
		} else if(((INavigatorTreeNode)obj).getNodeType()==INavigatorNodeTypeConstants.TYPE_TABLE_NODE){
			fileContent = DBOperator.getTableDDL((Table)((INavigatorTreeNode)obj).getData(),"oracle"); //$NON-NLS-1$
		} else {
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getShell(), getText(), Messages.ExportOracleDllAction_6);
			return;
