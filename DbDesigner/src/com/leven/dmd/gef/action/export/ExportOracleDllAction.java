...
public class ExportOracleDllAction extends SelectionAction {

	public static final String ID = "com.leven.dmd.gef.action.export.ExportOracleDllAction"; //$NON-NLS-1$

	private static final String[] EXTENSIONS = new String[] { "*.ddl", "*.sql", "*.txt"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

	public ExportOracleDllAction(IWorkbenchPart part) {
		super(part);
	...
	public void run() {
		...
		try {
			...
			MessageDialog.openError(getWorkbenchPart().getSite().getShell(),
					getText(), Messages.ExportOracleDllAction_6);
			return;
		}
		
		String fileContent = DbTableUtil.getSchemaDllContent(
				((SchemaDiagramEditor)getWorkbenchPart()).getSchema(),"oracle"); //$NON-NLS-1$
		...
		if (savePath != null) {
			...
			try {
				...
				mbox.setText(Messages.ExportOracleDllAction_8);
				mbox.setMessage(Messages.ExportOracleDllAction_9);
				mbox.open();
			} catch (IOException e) {
				e.printStackTrace();
			} finally{
