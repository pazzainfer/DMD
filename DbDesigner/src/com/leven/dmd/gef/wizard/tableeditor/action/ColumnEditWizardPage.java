...
public class ColumnEditWizardPage extends WizardPage {
	...
	private ColumnEditWizard wizard;
	...
	protected ColumnEditWizardPage(ColumnEditWizard wizard,Schema schema, Column column) {
		super("ColumnEditWizardPage"); //$NON-NLS-1$
		this.wizard = wizard;
		this.schema = schema;
		this.column = column;
		...
		this.setTitle(Messages.ColumnEditWizardPage_1);
		this.setMessage(Messages.ColumnEditWizardPage_2, 2);
	...
	public void createControl(Composite parent) {
		...
		inputButton.setText(Messages.ColumnEditWizardPage_3);
		inputButton.setLayoutData(new RowData(70,14));
		if(!wizard.isTempCanEdit()){
		...
		selectTemplateButton.setText(Messages.ColumnEditWizardPage_7);
		if(!wizard.isTempCanEdit()){
			selectTemplateButton.setEnabled(false);
		}
		if(optionFlag==1){
			selectTemplateButton.setSelection(true);
		}else {
			selectTemplateButton.setSelection(false);
		}
		...
		if(optionFlag==1){
			selectedText.setText(column.getColumnTemplate().getCode());
			selectedText.setEnabled(true);
		}else {
			selectedText.setEnabled(false);
		}
		...
		selectButton.setText(Messages.ColumnEditWizardPage_13);
		if(optionFlag==1){
			selectButton.setEnabled(true);
		}else {
			selectButton.setEnabled(false);
		...
		pkLab.setText(Messages.ColumnEditWizardPage_14);
		yesPkButton = new Button(pkComp, SWT.RADIO);
		yesPkButton.setText(Messages.ColumnEditWizardPage_15);
		yesPkButton.setSelection(column.isPk());
		
		noPkButton = new Button(pkComp, SWT.RADIO);
		noPkButton.setText(Messages.ColumnEditWizardPage_16);
		noPkButton.setSelection(!column.isPk());
		...
		columnNameLab.setText(Messages.ColumnEditWizardPage_17);
		columnNameLab.setLayoutData(labGrid);
		
		columnNameText = new Text(textGroup, SWT.BORDER);
		columnNameText.setLayoutData(textGrid);
		if(optionFlag==1 && !column.isTempCanEdit()){
			columnNameText.setEnabled(false);
		}
		columnNameText.setText(column.getName()==null?"":column.getName()); //$NON-NLS-1$
		...
		columnCnNameLab.setText(Messages.ColumnEditWizardPage_19);
		...
		columnCnNameText.setText(column.getCnName()==null?"":column.getCnName()); //$NON-NLS-1$
		...
		columnTypeLab.setText(Messages.ColumnEditWizardPage_21);
		...
		columnTypeCombo.setText(column.getType());
		...
		columnLengthLab.setText(Messages.ColumnEditWizardPage_24);
		...
		if(optionFlag==1){
			...
			if(ColumnType.hasLength(columnTypeCombo.getText())){
				columnLengthText.setEnabled(true);
			}
		}
		columnLengthText.setText(column.getLength()==0?"":(column.getLength()+"")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		columnScaleLab.setText(Messages.ColumnEditWizardPage_27);
		...
		if(optionFlag==1){
			...
			if(ColumnType.hasScale(columnTypeCombo.getText())){
				columnScaleText.setEnabled(true);
			}
		}
		columnScaleText.setText(column.getScale()==0?"":(column.getScale()+"")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		addToSchemaLink.setText(Messages.ColumnEditWizardPage_30);
		...
		columnDespLab.setText(Messages.ColumnEditWizardPage_0);
		...
		columnDespText.setText(column.getDescription()==null?"":column.getDescription()); //$NON-NLS-1$
	...
	private void updateComplete(){
		...
		if(optionFlag==0){
			if((message=validateInput())==null){
				this.setMessage(Messages.ColumnEditWizardPage_31, 1);
			...
			if(selectedText.getText().trim().equals("")){ //$NON-NLS-1$
				this.setMessage(Messages.ColumnEditWizardPage_33 + (optionFlag==1?Messages.ColumnEditWizardPage_34:Messages.ColumnEditWizardPage_35) + Messages.ColumnEditWizardPage_36, 3);
				this.setPageComplete(false);
			}else{
				this.setMessage(Messages.ColumnEditWizardPage_37, 1);
				this.setPageComplete(true);
			}
		}
	}
	...
	private String validateInput(){
		...
		if(columnName.equals("")){ //$NON-NLS-1$
			return Messages.ColumnEditWizardPage_39;
		}else if(!StringUtil.isRightDBName(columnName)){
			return Messages.ColumnEditWizardPage_4;
		}
		if(!column.getName().toLowerCase().equals(columnName.toLowerCase()) && 
				wizard.getColumnMap().containsKey(columnName.toLowerCase())){
			return Messages.ColumnEditWizardPage_40+columnName+Messages.ColumnEditWizardPage_41;
		}
		if(columnTypeCombo.getText().trim().equals("")){ //$NON-NLS-1$
			return Messages.ColumnEditWizardPage_43;
		}
		if(ColumnType.hasLength(columnTypeCombo.getText().trim())){
			if(columnLengthText.getText().trim().equals("")){ //$NON-NLS-1$
				return Messages.ColumnEditWizardPage_45;
			}else {
				String val = columnLengthText.getText().trim();
				if(!val.matches("[\\d]{1,}") || Double.parseDouble(val) <= 0){ //$NON-NLS-1$
					return Messages.ColumnEditWizardPage_47;
				}
			}
		}
		if(ColumnType.hasScale(columnScaleText.getText().trim())){
			if(columnScaleText.getText().trim().equals("")){ //$NON-NLS-1$
				return Messages.ColumnEditWizardPage_49;
			}else {
				String val = columnScaleText.getText().trim();
				if(!val.matches("[\\d]{1,}") || Double.parseDouble(val) <= 0){ //$NON-NLS-1$
					return Messages.ColumnEditWizardPage_51;
				}
			}
		}
		
		if(columnCnNameText.getText().trim().equals("")){ //$NON-NLS-1$
			return Messages.ColumnEditWizardPage_53;
		}
		if(!column.getName().equals(columnName) &&
				 wizard.getColumnMap().get(columnName)!=null){
			return Messages.ColumnEditWizardPage_54+columnName+Messages.ColumnEditWizardPage_55;
		}
		return null;
	}
	/**
		...
		if (optionFlag == 1) {
			...
			if (dialog.open() == Dialog.OK) {
				if ((template=dialog.getData()) != null) {
					selectedText.setText(template.getCode());
					if(column.getColumnTemplate()!=null && column.getColumnTemplate().getCode()
							.equals(template.getCode())){
						columnNameText.setEnabled(false);
						columnCnNameText.setEnabled(false);
					}else {
						columnNameText.setEnabled(true);
					...
					columnNameText.setText(template.getColumnName());
					columnCnNameText.setText(template.getColumnCnName());
	...
	private void addColumnToTemplate(){
		...
		if((message=validateInput())==null){
			...
			if(!schemaTemplate.addColumnTemplate(temp)){
				...
				mb.setText(Messages.ColumnEditWizardPage_61);
				mb.setMessage(Messages.ColumnEditWizardPage_62+columnNameText.getText().trim()+Messages.ColumnEditWizardPage_63);
				mb.open();
			}else {
				MessageBox mb = new MessageBox(this.getShell(), SWT.ICON_INFORMATION);
				mb.setText(Messages.ColumnEditWizardPage_64);
				mb.setMessage(Messages.ColumnEditWizardPage_65);
				mb.open();
	...
	public void setTypeSelected(boolean isTypeSelected) {
