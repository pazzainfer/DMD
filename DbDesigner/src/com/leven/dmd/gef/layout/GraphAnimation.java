	...
	public static boolean playbackState(IFigure container) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (rect2.width > rect1.width || rect2.height > rect1.height) {
				child.setBounds(new Rectangle((int) Math.round(progress
						* rect2.x + (1 - progress) * rect1.x), (int) Math
						.round(progress * rect2.y + (1 - progress) * rect1.y),
						rect2.width, rect2.height));
			} else {
				child.setBounds(new Rectangle((int) Math.round(progress
						* rect2.x + (1 - progress) * rect1.x), (int) Math
						.round(progress * rect2.y + (1 - progress) * rect1.y),
						(int) Math.round(progress * rect2.width
								+ (1 - progress) * rect1.width), (int) Math
